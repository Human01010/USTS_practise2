...
# 定义模型结果文件
model_result_files = {
    'TFT_BiLSTM': 'BiLSTM_transformer_results_c.npy',
    'LSTM': 'LSTM_results_c.npy',
    'CNN_LSTM_Skip': 'CNN_LSTM_Skip_results_c.npy'
}
...
for model_name, file_name in model_result_files.items():
    ...
    file_path = os.path.join(dir_path, file_name) # 假设结果文件也在 dir_path
    ...
    try:
        ...
        for record in results_array:
            ...
            # 检查 cycles 和 ground_truth 是否一致，如果之前已经加载过这个电池的数据
...
if not battery_names_to_plot:
    ...
    for i, battery_name in enumerate(battery_names_to_plot):
        ...
        for model_name_pred, prediction in model_predictions.items(): # 使用 model_name_pred 避免与外部循环变量冲突
            # 确保预测值与测试集的周期对齐
            ax.plot(test_cycles, prediction, label=model_name_pred, marker='x', linestyle='-')
