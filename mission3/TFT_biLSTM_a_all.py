...
# 单个name电池数据上随机选择60%数据作为训练数据，20%作为验证，20%作为测试
def get_train_test_a(data_dict, name, window_size=8):
    data_sequence = data_dict[name][1:]
    # 随机选取data_sequence的60%作为训练数据，20%作为验证数据，20%作为测试数据
    # 创建 (feature, target) 对
    x_all, y_all = build_instances(data_sequence, window_size)
    x_all = np.transpose(x_all, (0, 2, 1)) # 交换轴 1 和 2
    
    if len(x_all[0]) == 0: # 如果数据太少，无法创建实例
        return np.array([]), np.array([]), np.array([]), np.array([]), np.array([]), np.array([])

    # 如果需要保持序列顺序，则不能直接用 train_test_split 对原始 sequence 分割而是对 build_instances 之后的结果 x_all, y_all 进行分割
...
def train_val_test(lr, window_size, lstm_hidden_size, lstm_num_layers,
    ...
    # Loop through all batteries to split into train/test and collect
    for name in Battery_list:
        ...
        all_eval_x_list.append(val_x)
        all_eval_y_list.append(val_y)
    ...
    all_eval_x = np.concatenate(all_eval_x_list, axis=0)
    all_eval_y = np.concatenate(all_eval_y_list, axis=0)

    print(f"\nCombined training data shape: X={all_train_x.shape}, y={all_train_y.shape}")
    print(f"Combined evaluation data shape: X={all_eval_x.shape}, y={all_eval_y.shape}")

    #Calculate Standardization Statistics from Combined Training Data ---
    ...
    print(f"Feature means (per feature): {feature_means}")
    print(f"Feature stds (per feature): {feature_stds}")
    print(f"Target mean: {target_mean}")
    print(f"Target std: {target_std}")

    #Calculate Standardization Statistics from Combined evaluation Data ---
    print("Calculating standardization statistics from combined evaluation data...")
    eval_feature_means, eval_feature_stds, eval_target_mean,eval_target_std = cal_feature_target_mean_std(all_eval_x, all_eval_y)
    ...
    # Standardize the Combined Evaluation Data
    eval_x_scaled = (all_eval_x - eval_feature_means) / eval_feature_stds
    eval_y_scaled = (all_eval_y - eval_target_mean) / eval_target_std

    X_train_tensor = torch.from_numpy(train_x_scaled).float().to(device) # Ensure float()
    y_train_tensor = torch.from_numpy(train_y_scaled.reshape(-1, 1)).float().to(device) # Ensure float()

    dataset = TensorDataset(X_train_tensor, y_train_tensor)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)

    # eval dataset
    X_eval_tensor = torch.from_numpy(eval_x_scaled).float().to(device) # Ensure float()
    y_eval_tensor = torch.from_numpy(eval_y_scaled.reshape(-1, 1)).float().to(device) # Ensure float()
    eval_dataset = TensorDataset(X_eval_tensor, y_eval_tensor)
    eval_dataloader = DataLoader(eval_dataset, batch_size=batch_size, shuffle=False)

    actual_input_features = all_train_x.shape[-1] # Last dimension is number of features


    # Initialize the model *once*
    model = LSTM_TFT(
        input_features=actual_input_features, # Number of features per timestep
    ...
    # Initialize optimizer and criterion *once*
    ...
    for epoch in range(epochs):
        ...
        # if (epoch + 1) % 100 == 0 or epoch == 1:
            # evaluate on the evaluation set every 100 epochs
        model.eval()
        with torch.no_grad():
            eval_loss = 0
            all_eval_preds_scaled_list = []
            all_eval_gt_scaled_list = []
            for eval_batch_X, eval_batch_y in eval_dataloader:
                eval_output = model(eval_batch_X)
                eval_loss += criterion(eval_output, eval_batch_y).item()
                all_eval_preds_scaled_list.append(eval_output.cpu().numpy())
                all_eval_gt_scaled_list.append(eval_batch_y.cpu().numpy())
            
            avg_eval_loss = eval_loss / len(eval_dataloader)

            # Concatenate all predictions and ground truths
            all_eval_preds_scaled = np.concatenate(all_eval_preds_scaled_list).reshape(-1)
            all_eval_gt_scaled = np.concatenate(all_eval_gt_scaled_list).reshape(-1)

            # De-standardize predictions and ground truths
            # target_mean and target_std are from the combined training data
            eval_preds_original = all_eval_preds_scaled * target_std + target_mean
            eval_gt_original = all_eval_gt_scaled * target_std + target_mean

            eval_mae, eval_mse, eval_rmse, eval_r2, eval_mape = evaluation(y_test=eval_gt_original, y_predict=eval_preds_original)
            eval_losses.append(eval_mse) 
            if (epoch + 1) % 100 == 0 or epoch == 1:
                # Calculate evaluation metrics
                print(f'Epoch [{epoch+1}/{epochs}], Training Loss: {avg_loss:.6f}, Evaluation Loss: {avg_eval_loss:.6f}')
                print(f'Evaluation Metrics: MAE:{eval_mae:<6.4f} | MSE:{eval_mse:<6.6f} | RMSE:{eval_rmse:<6.6f} | R2:{eval_r2:<6.4f} | MAPE:{eval_mape:<6.4f}%')
            model.train() # Set model back to training mode

    print("--- Training complete ---")

    print("\n--- test trained model on individual battery test sets ---")

    score_list = [] # Store evaluation scores per battery
    all_test_results = [] # Store prediction results per battery

    # Loop through the test data collected earlier for each battery
    ...
    with torch.no_grad(): # Disable gradient calculation for evaluation
        for battery_test_data in individual_test_data_list:
            ...
            print(f'Evaluating on battery test data: {name}...')
            test_x_scaled = (test_x - feature_means) / feature_stds
            X_test_tensor = torch.from_numpy(test_x_scaled).float().to(device) # Ensure float()
    ...
    return score_list, [losses], all_test_results,eval_losses
...
score_list ,loss_list, all_test_results, eval_losses = train_val_test(
    lr=lr,
...
print("\n--- Overall Test RMSE Scores ---")
for i, name in enumerate(Battery_list):
    print(f"Scores for {name}: MAE:{score_list[i][0]:<6.4f} | MSE:{score_list[i][1]:<6.4f} | RMSE:{score_list[i][2]:<6.4f} | R2:{score_list[i][3]:<6.4f} | MAPE:{score_list[i][4]:<6.4f}%")
...
combined_losses = loss_list[0] # Get the actual list of losses
