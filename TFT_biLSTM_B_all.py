...
# 单个name电池数据中前80% circle数据作为训练数据，最后20% circle数据作为测试数据
def get_train_test_b(data_dict, name, window_size=8):
    ...
    # 计算分割点，保持序列顺序
    split_idx = int(len(x_all) * 0.8) # 前80%作为训练数据
    ...
    # 确保即使分割后，如果某一部分为空，也返回空数组而不是None或引起错误
    if len(train_x) == 0:
...
def train_val_test(lr, window_size, lstm_hidden_size, lstm_num_layers,
    ...
    for name in Battery_list:
        ...
        # get_train_test_b still splits data *within* each battery based on percentage
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 100 == 0 or epoch == 1: # Print progress every 100 epochs or after first epoch
    ...
    with torch.no_grad(): # Disable gradient calculation for evaluation
        ...
        for battery_test_data in individual_test_data_list:
            ...
            print(f'Prediction on battery test data: {name}...')
...
lstm_num_layers = 1   # Number of LSTM layers 1
...
np.save("BiLSTM_Transformer_results_b.npy", all_test_results)
