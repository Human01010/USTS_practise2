...
class Net(nn.Module):
    def __init__(self, input_size=3, hidden_dim=8, num_layers=1, mode='LSTM'):
        ...
        # The cell expects input_size, hidden_size, num_layers
        # batch_first=True means input/output shape is (batch, seq_len, features)
        if mode == 'LSTM':
            ...
            raise ValueError("mode must be 'RNN', 'LSTM', or 'GRU'")

        self.linear = nn.Linear(hidden_dim, 1) # Output a single value (predicted capacity)

    def forward(self, x):
        # x shape: (batch_size, sequence_length, input_size)
        # e.g., (batch_size, window_size, 3) if input_size is 3 and seq_len is window_size

        # self.cell returns:
        # output: (batch_size, sequence_length, hidden_dim * num_directions) (num_directions is 1 here)
        # hidden: tuple (h_n, c_n) for LSTM, (h_n,) for GRU/RNN. h_n shape: (num_layers * num_directions, batch_size, hidden_dim)
        out, _ = self.cell(x)

        # Take the output from the *last time step* of the sequence
        # out[:, -1, :] selects the last element along the sequence dimension (dim 1)
        # Resulting shape: (batch_size, hidden_dim)
        out = out[:, -1, :]

        # Apply linear layer to the last step's output to get the final prediction
...
np.save("LSTM_results_b.npy", all_test_results)
