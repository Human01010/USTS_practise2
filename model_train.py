...
def dataloader(batch_size, workers=0):
    ...
    # 确保数据是三维的 batch_size, seq_len, feature_dim 模型需要
    ...
    # 将三维数据转换为二维数据进行归一化 sklearn库需要 batch_size*seq_len, feature_dim
...
def model_train(epochs, model, optimizer, loss_function, train_loader, val_loader, device, scheduler=None):
    ...
    plt.tight_layout()
    plt.savefig('model/loss_curve.png', dpi=300)
